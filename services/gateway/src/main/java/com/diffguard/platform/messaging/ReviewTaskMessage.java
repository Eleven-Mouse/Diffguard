...
import com.diffguard.review.ReviewEngineFactory.EngineType;
...
public class ReviewTaskMessage {
    ...
    public ReviewTaskMessage(String mode, ReviewConfig config,
        ...
        this(UUID.randomUUID().toString(), mode, config, diffEntries, projectDir, toolServerUrl, null, hotfix);
    }

    public ReviewTaskMessage(String taskId, String mode, ReviewConfig config,
                             List<DiffFileEntry> diffEntries, String projectDir,
                             String toolServerUrl, boolean hotfix) {
        this(taskId, mode, config, diffEntries, projectDir, toolServerUrl, null, hotfix);
    }

    public ReviewTaskMessage(String taskId, String mode, ReviewConfig config,
                             List<DiffFileEntry> diffEntries, String projectDir,
                             String toolServerUrl, List<String> allowedFiles, boolean hotfix) {
        this.taskId = taskId;
        this.mode = mode;
        this.config = config;
        ...
        this.allowedFiles = allowedFiles == null ? List.of() : List.copyOf(allowedFiles);
        this.hotfix = hotfix;
    ...
    public byte[] toJsonBytes() {
        try {
            ...
            if (allowedFiles != null && !allowedFiles.isEmpty()) {
                allowedFiles.forEach(allowedArray::add);
            } else {
    ...
    public static ReviewTaskMessage fromJson(byte[] data) {
        try {
            ...
            String taskId = extractTaskId(data);
            ...
            JsonNode allowedNode = root.path("allowed_files");
            if (allowedNode.isArray()) {
                for (JsonNode f : allowedNode) {
                    String raw = f.asText("");
                    if (raw != null && !raw.isBlank()) {
                        allowed.add(raw.trim());
                    }
                }
            }

            return new ReviewTaskMessage(taskId,
    ...
    public static String extractTaskId(byte[] data) {
        ...
        try {
            ...
            return textOrBlank(root.path("request_id"));
        } catch (Exception e) {
            return null;
        }
    }
