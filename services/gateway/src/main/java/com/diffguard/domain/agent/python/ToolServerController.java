...
import com.diffguard.domain.agent.python.ToolSessionManager.Session;
...
public class ToolServerController {
    ...
    private ObjectNode successResponse(String result) {
        ObjectNode node = JacksonMapperHolder.getMapper().createObjectNode();
    ...
    /**
     * 内部持有 Jackson ObjectMapper 单例，避免重复创建。
     */
    private static class JacksonMapperHolder {
        private static final com.fasterxml.jackson.databind.ObjectMapper MAPPER =
                new com.fasterxml.jackson.databind.ObjectMapper();
        static com.fasterxml.jackson.databind.ObjectMapper getMapper() { return MAPPER; }
    }

    /**
     * 用于 stream JsonNode 数组的辅助。
     */
    private static class StreamSupport {
        static java.util.stream.Stream<JsonNode> stream(java.util.Spliterator<JsonNode> spliterator, boolean parallel) {
