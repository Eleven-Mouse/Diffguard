...
import com.diffguard.review.ast.ASTAnalyzer;
import com.diffguard.review.ast.model.ASTAnalysisResult;
...
public class CodeRAGService {
    ...
    private static VectorStore createVectorStore(ReviewConfig config) {
        ReviewConfig.CodeRagStoreConfig storeConfig = config.getCoderagStore();
        if (storeConfig == null || !storeConfig.isChromaProvider()) {
            log.info("Code RAG vector store provider is not chromadb, force using ChromaVectorStore with default config");
            ReviewConfig.CodeRagStoreConfig defaultCfg = new ReviewConfig.CodeRagStoreConfig();
            defaultCfg.setProvider("chromadb");
            return new ChromaVectorStore(defaultCfg);
        }
        return new ChromaVectorStore(storeConfig);
    }
