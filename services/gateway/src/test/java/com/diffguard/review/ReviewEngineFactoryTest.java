...
class ReviewEngineFactoryTest {
    ...
    class ResolveToolServerUrl {
        ...
        @Test
        @DisplayName("uses toolService URL when configured")
        void usesToolServiceUrlWhenConfigured() {
            ReviewConfig config = new ReviewConfig();
            ReviewConfig.ToolServiceConfig toolServiceConfig = new ReviewConfig.ToolServiceConfig();
            toolServiceConfig.setUrl("http://tool.internal:19090/");
            config.setToolService(toolServiceConfig);

            String url = ReviewEngineFactory.resolveToolServerUrl(config);
            assertEquals("http://tool.internal:19090", url);
        }

        @Test
        @DisplayName("uses toolService host/port when agentService not configured")
        void usesToolServiceHostPortWhenNoAgentService() {
            ReviewConfig config = new ReviewConfig();
            ReviewConfig.ToolServiceConfig toolServiceConfig = new ReviewConfig.ToolServiceConfig();
            toolServiceConfig.setHost("toolsvc");
            toolServiceConfig.setPort(19091);
            config.setToolService(toolServiceConfig);

            String url = ReviewEngineFactory.resolveToolServerUrl(config);
            assertEquals("http://toolsvc:19091", url);
        }
