"""Security review agent - detects vulnerabilities and security issues."""
...
class SecurityAgent(ReviewAgent):
    ...
    def description(self) -> str:
        return "Detects security vulnerabilities: SQL injection, XSS, auth issues, secret exposure, insecure deserialization."
    ...
    async def review(
        ...
        system = _load_prompt("reviewagents/security-system.txt")
        if focus_areas:
...
def _parse_agent_output(llm: Any, output_text: str) -> AgentReviewResult:
    """Parse LLM agent output into AgentReviewResult with fallback."""
    ...
    try:
        ...
        return AgentReviewResult(summary="Failed to parse structured output", highlights=[output_text[:200]])
