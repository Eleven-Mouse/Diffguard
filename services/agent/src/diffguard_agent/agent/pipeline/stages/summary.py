...
class SummaryStage(PipelineStage):
    ...
    async def execute(self, context: PipelineContext) -> PipelineContext:
        ...
        if context.input.historical_context:
            user = user.replace("{{historical_context}}", context.input.historical_context)
        else:
            user = user.replace("{{historical_context}}", "（无历史审查记录）")

        structured_llm = context.input.llm.with_structured_output(_DiffSummary)
        ...
        context.summary.estimated_risk_level = result.estimated_risk_level
        context.summary.file_groups = _normalise_file_groups(
            result.file_focus, context.summary.changed_files,
        )

        logger.info("Summary complete: %d files, risk=%d, groups=%s",
                    len(result.changed_files), result.estimated_risk_level,
                    {k: len(v) for k, v in context.summary.file_groups.items()})
        return context
