...
class StaticRulesStage(PipelineStage):
    ...
    async def execute(self, context: PipelineContext) -> PipelineContext:
        ...
        for line_no, line in enumerate(context.input.diff_text.split("\n"), 1):
            ...
            for pattern, issue_type, severity, message in _RULES:
                if pattern.search(content):
                    ...
                    file_path = self._extract_file_path(context.input.diff_text, line_no)
        ...
        if issues:
            context.review.review_results["static_rules"] = [
