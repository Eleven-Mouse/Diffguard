...
from diffguard_agent.agent.llm_utils import load_prompt, sanitize_diff_for_prompt
...
class ReviewerStage(PipelineStage):
    ...
    async def execute(self, context: PipelineContext) -> PipelineContext:
        ...
        tasks = [
            self._run_reviewer(
                context.input.llm, name, sys_file, usr_file,
                context.summary.summary, context.input.diff_text, context.input.tool_client,
                file_diffs=context.input.file_diffs or None,
                file_group=context.summary.file_groups.get(name),
            )
        ...
        for (name, _, _), result in zip(self._reviewers, results):
            if isinstance(result, Exception):
                ...
                context.review.review_results[name] = _TargetedReviewResult(
